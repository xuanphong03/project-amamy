...
class VGSE_Provider_Custom_table extends VGSE_Provider_Abstract {
	...
	function _get_rows( $args ) {
		...
		foreach ( $this->get_arg( 'columns', $args['post_type'] ) as $column_key => $column ) {
			if ( in_array( $column_key, $reserved_keys, true ) ) {
				continue;
			}
	...
	function get_meta_field_unique_values( $meta_key, $post_type = null ) {
		...
		$sql    = $wpdb->prepare( 'SELECT m.%i FROM %i m WHERE m.meta_key = %s GROUP BY m.%i ORDER BY LENGTH(m.%i) DESC LIMIT 4', $meta_value_column, $post_meta_table, $meta_key, $meta_value_column, $meta_value_column );
		$values = apply_filters( 'vg_sheet_editor/provider/custom_table/meta_field_unique_values', $wpdb->get_col( $sql ), $meta_key, $post_type );
