...
if ( ! class_exists( 'WP_Sheet_Editor_Infinite_Serialized_Field' ) ) {
	...
	class WP_Sheet_Editor_Infinite_Serialized_Field {
		...
		function remove_empty_arrays( $value ) {
			...
			if ( ! method_exists( VGSE()->helpers, 'array_depth' ) ) {
				return $value;
			}

			$depth = VGSE()->helpers->array_depth( $value );
			if ( $depth === 1 ) {
				...
				foreach ( $value as $index => $single_value ) {
					...
					$value[ $index ] = $this->remove_empty_arrays( $single_value );
				}
			}
			return $value;
		}
		...
		function update_value( $post_id, $key, $value ) {
			...
			$value = $this->remove_empty_arrays( $value );
