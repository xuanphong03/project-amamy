	...
	function get_file_curl( $url, $fullpath, $to_variable = false, $iteration = false ) {
		...
		$response = wp_remote_get( $url, array(
			...
			'headers' => array(
				...
				'Accept' => 'application/xml;q=1.0, */*;q=0.8'
			),
		) );
...
if ( ! function_exists( 'pmxi_curl_download' ) ) {
	...
	function pmxi_curl_download( $url, $fullpath, $to_variable ) {
		...
		curl_setopt( $ch, CURLOPT_HTTPHEADER, array( 'User-Agent: WP All Import (version:'.PMXI_VERSION.')', 'Accept: application/xml;q=1.0, */*;q=0.8' ));
