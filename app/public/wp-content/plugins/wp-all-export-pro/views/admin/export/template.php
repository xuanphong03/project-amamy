	...
	<tr>
		<td class="left">		
			...
			<form class="wpallexport-template <?php echo ! $this->isWizard ? 'edit' : '' ?> wpallexport-step-3" method="post" style="display:none;" id="templateForm">
													...
													foreach ($init_fields as $k => $field) {
														...
														<li>
															<div class="custom_column" rel="<?php echo ($i + 1);?>">
																<label class="wpallexport-xml-element"><?php echo esc_html(XmlExportEngine::sanitizeFieldName($field['name'])); ?></label>
																<input type="hidden" name="ids[]" value="1"/>
																...
																<input type="hidden" name="cc_name[]" value="<?php echo (strtoupper($field['name']) == 'ID' && (XmlExportEngine::$exportOptions['export_to_sheet'] === 'xlsx' || XmlExportEngine::$exportOptions['export_to_sheet'] === 'xls')) ? 'id' : esc_attr($field['name']);?>"/>
									...
									<div class="wpallexport-collapsed-content-inner">
										...
										<div class="input">
											...
											<div class="input" style="margin: 10px 0;">
												...
												<label for="simple_custom_xml_cdata_logic_auto"><?php esc_attr_e('Automatically wrap data in CDATA tags when it contains illegal characters', 'wp_all_export_plugin') ?></label>
											</div>
											<div class="input" style="margin: 10px 0;">
												...
												<label for="simple_custom_xml_cdata_logic_all"><?php esc_attr_e('Always wrap data in CDATA tags', 'wp_all_export_plugin') ?></label>
											</div>
											<div class="input" style="margin: 10px 0;">
												...
												<label for="simple_custom_xml_cdata_logic_never"><?php esc_attr_e('Never wrap data in CDATA tags', 'wp_all_export_plugin') ?></label>
												<div class="switcher-target-simple_custom_xml_cdata_logic_never" style="padding-left:28px;">
			                    ...
			                    <div class="wp-all-export-wpml-options">
							        ...
							        <div class="input">
							            <?php foreach ($wpml_options as $key => $value):?>
							                <div class="input">
							                    <input type="radio" class="wpml_lang" id="<?php echo esc_attr($random); ?>_wpml_lang_<?php echo esc_attr($key);?>" name="<?php echo esc_attr($random); ?>_wpml_lang" value="<?php echo esc_attr($key); ?>" <?php if ($post['wpml_lang'] == $key):?>checked="checked"<?php endif; ?> class="switcher"/>
							                    <label for="<?php echo esc_attr($random); ?>_wpml_lang_<?php echo esc_attr($key);?>"><?php echo wp_kses_post($value); ?></label>
							                </div>
							            <?php endforeach; ?>
							        </div>
							    </div>
			    ...
			    <input type="hidden" id="wpml_lang" name="wpml_lang" value="<?php echo esc_attr($post['wpml_lang']);?>" />
			    <?php endif;?>
											...
											<div class="input">
												<select name="xml_template_type" class="xml_template_type">
													...
													if(in_array('product', $post['cpt'])) {
														?>
														<option value="<?php echo esc_attr(XmlExportEngine::EXPORT_TYPE_GOOLE_MERCHANTS); ?>" <?php if ($post['xml_template_type'] == XmlExportEngine::EXPORT_TYPE_GOOLE_MERCHANTS) echo 'selected="selected"';?>><?php esc_html_e('Google Merchant Center Product Feed', 'wp_all_export_plugin'); ?></option>
													<?php
													}
													?>
												</select>
				...
				<div class="input wpallexport-section load-template-container" style="padding-left: 8px;">
					...
					<p style="margin: 11px; float: left;">
						...
						<label for="save_template_as"><?php esc_html_e('Save settings as a template','wp_all_export_plugin');?></label>
					</p>
					<div class="switcher-target-save_template_as" style="float: left; overflow: hidden;">
						<input type="text" name="name" placeholder="<?php esc_html_e('Template name...', 'wp_all_export_plugin') ?>" style="vertical-align:middle; line-height: 26px;" value="<?php echo esc_attr($post['name']) ?>" />
					</div>				
				...
				<hr>
                <div class="input custom_xml_upgrade_notice wpallexport-free-edition-notice wpallexport-google-merchants-template" style="vertical-align:middle; position: relative; top: -5px;">
					<span class="" style="margin: 0 0 10px;">
						...
						<a class="upgrade_link" target="_blank" href="https://www.wpallimport.com/portal/discounts/?utm_source=export-plugin-pro&utm_medium=upgrade-notice&utm_campaign=export-google-merchants"><?php esc_html_e('Click here to purchase the WooCommerce Export Add-On','wp_all_export_plugin');?></a>
					</span>
                </div>
...
<fieldset class="optionsset column rad4 wp-all-export-custom-xml-help">
    ...
    <div class="title"><span style="font-size:1.5em;"
            ...
            class="wpallexport-edit-row-title"><?php esc_html_e('Edit Export Field', 'wp_all_export_plugin'); ?></span></div>
